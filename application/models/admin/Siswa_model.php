...
class Siswa_model extends CI_Model
{
	...
	public function list_laporan($start = null)
	{
		...
		$this->db->where('xx_pendaftaran.status', 1);
		if ($start != null) {
			$this->db->where("date_format(xx_pendaftaran.created_at,'%Y-%m')",$start);
	...
	public function delete_siswa_pendaftaran($id)
	{
		$this->db->where('id_user', $id);
		$this->db->delete('xx_pendaftaran');
		return true;
	...
	public function detail_siswa($id)
	{
		...
		$this->db->join('xx_profile', 'xx_profile.id_user = xx_users.id_user','LEFT');
